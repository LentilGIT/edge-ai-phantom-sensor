"""
shap_analysis_320bin.py

SHAP-based input importance analysis for the deployed 320-bin DNN model.

Model architecture : 320 -> 128 -> 64 -> 32 -> 1
Input range        : FFT bin1 – bin320 (47Hz – 15kHz)
Sampling rate      : 48kHz, 1024-point Hanning FFT
Frequency resolution: 46.875 Hz/bin

This script loads pre-extracted model weights (.npz) and FFT CSV data,
computes SHAP values using KernelExplainer, and visualizes which frequency
bands actually contribute to pressure estimation.

Key finding:
    Weight magnitude analysis highlighted the 14–15kHz band,
    but SHAP analysis reveals that the 1.1–1.5kHz band
    (pump rotation frequency) dominates the actual prediction output.

Dependencies:
    pip install numpy matplotlib shap

Usage:
    1. Place model_weights_320bin.npz in the same folder as this script.
    2. Set CSV_FOLDER to the directory containing FFT CSV files (TEST5 data).
    3. Run: python shap_analysis_320bin.py
    4. Output: shap_result_320bin.png saved in the same folder.
"""
...
# ── Configuration ─────────────────────────────────────────────────────────────
NPZ_PATH   = "model_weights_320bin.npz"   # Path to extracted model weights
...
def predict(X):
    ...
    Architecture: 320 -> 128 (BN+ReLU) -> 64 (BN+ReLU) -> 32 (ReLU) -> 1
    Input X shape: (n_samples, 320)
    Output shape : (n_samples, 1)
    """
    X = np.array(X, dtype=np.float32)
    # Layer 1: 320 -> 128 with BatchNorm and ReLU
    h = X @ w['W1'] + w['b1']
    h = batch_norm(h, w['bn1_mean'], w['bn1_var'], w['bn1_gamma'], w['bn1_beta'])
    h = relu(h)
    # Layer 2: 128 -> 64 with BatchNorm and ReLU
    h = h @ w['W2'] + w['b2']
...
# Load CSV files (bin1–320: exclude bin0 DC component)
...
for f in csv_files:
    d = np.loadtxt(f)
    data.append(d[1:321])  # bin1 to bin320
...
# Frequency axis: bin1–320 → 47Hz–15kHz
freq_res = 48000 / 1024          # 46.875 Hz/bin
freqs    = np.arange(1, 321) * freq_res / 1000  # kHz
...
plt.title('SHAP Value Analysis — Input Importance (bin1–320, 47Hz–15kHz)\n'
          'Deployed Model: 320→128→64→32→1  R²=0.9946  RMSE=3.18kPa')
...
output_path = 'shap_result_320bin.png'
